...
Die TypeScript-Umsetzung in Listing \ref{lst:ts-dijkstra} erfordert durch die explizite Typisierung auf den ersten Blick etwas mehr strukturellen Aufwand. Die Signaturen der Übergabeparameter (\texttt{Record<string, Array<[string, number]>>}) dokumentieren exakt die erwartete Graphenstruktur. Für die Menge der bereits besuchten Knoten (\texttt{visited}) wird der in TypeScript nativ vorhandene Datentyp \texttt{Set<string>} verwendet. Dies stellt die direkte und idiomatische Entsprechung zu Pythons \texttt{set} dar. Der Algorithmus bleibt somit in seiner Kernlogik sehr nah am Python-Original. Obwohl der syntaktische Schreibaufwand durch die strengen Typdeklarationen im Vergleich zur kompakten Python-Version leicht steigt, zahlt sich dies durch eine signifikant höhere statische Sicherheit aus.
...
\end{minted}
\captionof{listing}{TypeScript-Beispiel: shortestPath mit nativem Set}
...
In TypeScript wurde dieselbe algorithmische Idee durch den Basis-Promt Prompt zuerst deutlich imperativer abgebildet. Der Code greift auf die \texttt{recursive-set}-Bibliothek zurück, da TypeScript native Set-Comprehensions für tiefgreifende Wertvergleiche nicht unterstützt:
...
Wenn man die KI jedoch dazu auffordert, deutlich deklarativeren Code zu generieren, ähnelt die Schreibweise und Länge des Codes der Python-Version aus \ref{pythonreduce}:
