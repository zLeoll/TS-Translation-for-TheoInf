...
\subsubsection{Laufzeitvergleich: \texorpdfstring{\(n\)}{n}-Damen (Davis--Putnam mit JW-Heuristik)}
Das \emph{8-Damen-Problem} wird in beiden Versionen als SAT-Problem in KNF kodiert und anschließend mit dem Davis--Putnam-Verfahren gelöst, wobei die Literalwahl durch die Jeroslow--Wang-Heuristik gesteuert wird. % [file:78][file:80][file:77]
Da \(n=8\) in der Praxis sehr schnell lösbar ist, wurde für eine aussagekräftigere Messung \(n=16\) gewählt, da hier die Heuristik und die Datenstrukturen deutlich stärker beansprucht werden. % [file:78][file:80]

\paragraph{Messung in Python.}
Analog zum Merge-Sort-Vergleich wurde im Python-Notebook die Jupyter-Messfunktion verwendet, die \emph{CPU time} und \emph{Wall time} ausgibt. % [file:78]
\Needspace{5\baselineskip}
\begin{minted}{python}
	%%time
	Solution = queens(16)
...
\paragraph{Messung in TypeScript (Node.js/V8).}
In der TypeScript-Version wurde die Laufzeit mit \texttt{console.time()} und \texttt{console.timeEnd()} gemessen; diese Messung entspricht der real verstrichenen Zeit. % [file:80]
\Needspace{6\baselineskip}
\begin{minted}{typescript}
	console.time("queens");
...
Für \(n=16\) zeigt sich ein deutlicher Performance-Vorteil der TypeScript/Node.js-Ausführung: \(17{,}041\,\mathrm{s}\) gegenüber \(24{,}8\,\mathrm{s}\) in Python, also rund \(31\,\%\) weniger \emph{Wall time}. % [file:78][file:80]
Der Unterschied ist bei \(n=8\) erwartungsgemäß weniger relevant, da das Problem bereits unter einem Zehntel einer Sekunde lösbar ist und damit Mess-Overhead dominiert. % [file:78][file:80]

\paragraph{Einordnung über Laufzeitumgebungen.}
Node.js führt den kompilierten JavaScript-Code auf der V8-Engine aus, die häufig ausgeführte Codepfade zur Laufzeit optimieren kann, während Python den Algorithmus primär im Interpreter ausführt. % [file:80][file:78]
Gerade beim Davis--Putnam-Verfahren treten wiederkehrende Iterationsmuster auf (z.\,B.\ in \texttt{saturate} und \texttt{reduce}), wodurch V8 in „hot paths“ stärker profitieren kann als eine bytecode-interpretierte Ausführung. % [file:79][file:77]

\paragraph{Hauptgrund: Hash-basierte \texttt{RecursiveSet}-Struktur.}
Ein zentraler Beschleuniger in der TypeScript-Version ist die verwendete \texttt{recursive-set}-Bibliothek, die \emph{Value Semantics}, Deep-Equality und Hashing für rekursive Strukturen (z.\,B.\ Mengen von Mengen) bereitstellt. % [file:76]
Die Datenstruktur implementiert Hash-Tabellen mit \emph{Open Addressing} und \emph{Linear Probing} sowie eine auf Performance ausgelegte Hash-Funktion; grundlegende Operationen wie \texttt{add}/\texttt{has}/\texttt{remove} sind damit amortisiert \(O(1)\). % [file:76]
Zusätzlich nutzt \texttt{RecursiveSet} flache Arrays für bessere Cache-Lokalität und führt eine „Frozen“-Logik ein, sobald der Hashwert berechnet wurde, um die Hash-Stabilität sicherzustellen. % [file:76]

\paragraph{Warum das hier besonders wirkt.}
Der Davis--Putnam-Code erzeugt und verarbeitet sehr viele Klauselmengen und Unit-Klauseln; in beiden Implementierungen sind genau diese Mengenoperationen der dominierende Kostenfaktor (z.\,B.\ \texttt{reduceClauses} und die Schleife in \texttt{saturateClauses}). % [file:79][file:77]
In TypeScript werden Klauseln/Literale dabei konsequent als \texttt{RecursiveSet} bzw.\ \texttt{Tuple} modelliert und über Hashing sowie strukturelle Gleichheit verglichen, was die vielen Membership-Checks und das Duplikat-Management effizient hält. % [file:79][file:76]
In Python wird dieselbe Idee mit \texttt{frozenset} (Klausel) und \texttt{set}/\texttt{frozenset} (Klauselmenge) umgesetzt; dabei entsteht jedoch zusätzlicher Overhead durch die interpreterseitige Steuerlogik und das häufige Erzeugen neuer (frozen) Sets in Comprehensions und in der Rekursion. % [file:77][file:78]

\paragraph{Bezug zu den Bibliotheks-Voraussetzungen.}
Die im \texttt{recursive-set}-Modul formulierten Voraussetzungen (keine \texttt{NaN}/\texttt{Infinity}, keine „plain objects“, gute Hash-Verteilung, Unveränderlichkeit des Hashcodes nach dem Einfügen und keine Zyklen) sind in diesem SAT-Use-Case erfüllt, da Literale und Variablen als Strings bzw.\ \texttt{Tuple}-Objekte modelliert werden und die erzeugten Klauseln nach der Konstruktion nicht mutiert werden. % [file:76][file:79][file:80]
Damit bleiben die zugesicherten Hash-basierten Laufzeiteigenschaften in der Praxis stabil und wirken sich insbesondere bei \(n=16\) sichtbar auf die Gesamtlaufzeit aus. % [file:76][file:78][file:80]

\subsubsection{Warum der Unterschied hier kleiner ist als bei Merge-Sort}
Im Vergleich zum Merge-Sort-Benchmark fällt der Performance-Abstand beim \(n\)-Damen-Problem geringer aus, weil sich die dominierenden Kosten stärker in Richtung Datenstruktur-Operationen und Speicherverwaltung verschieben.
Bei Davis--Putnam entstehen in kurzer Zeit sehr viele temporäre Klauselmengen, Unit-Klauseln und Reduktionen, d.\,h.\ die Laufzeit wird weniger von „tight loops“ und stärker von \texttt{add}/\texttt{has}/\texttt{union} sowie dem Erzeugen kleiner Mengenobjekte geprägt. % [file:77][file:79]

Ein wichtiger Punkt ist, dass Python bei diesem Workload nicht nur „reines Python“ ausführt, sondern massiv von hochoptimierten, in C implementierten Hash-Tabellen für \texttt{set} und \texttt{frozenset} profitiert.
Insbesondere sind zentrale Operationen wie Membership-Checks und das Hashing von \texttt{frozenset} in CPython direkt in \texttt{setobject.c} implementiert; zudem wird der Hashwert eines \texttt{frozenset} intern gecacht, was wiederholte Hash-Berechnungen reduziert. % [web:85]

Die TypeScript-Version verwendet zwar mit \texttt{RecursiveSet} ebenfalls eine hash-basierte Datenstruktur mit Open Addressing und Linear Probing, jedoch bleibt sie eine Bibliotheksimplementierung auf JavaScript-Ebene, die zusätzlich Deep-Equality und eigenes Hashing für rekursive Strukturen bereitstellt.
Diese zusätzlichen Abstraktionen sind für die korrekte „Value Semantics“ entscheidend, können aber bei sehr vielen kurzlebigen Objekten (z.\,B.\ vielen kleinen Klauseln) den Anteil von Allokationen und damit den Druck auf den Garbage Collector erhöhen. % [file:76][web:87]

Hinzu kommt, dass die JIT-Optimierung der V8-Engine bei stark verzweigtem, rekursivem Suchcode mit vielen kleinen Objekten und dynamischen Kontrollflüssen typischerweise weniger stark „durchschlägt“ als bei regelmäßigem, zahlenlastigem Code, wie er bei Sortierbenchmarks häufiger entsteht.
Dadurch bleibt der Vorteil von Node.js/V8 zwar klar sichtbar, fällt aber im SAT-basierten \(n\)-Damen-Szenario insgesamt geringer aus als im Merge-Sort-Vergleich. % [web:87]
